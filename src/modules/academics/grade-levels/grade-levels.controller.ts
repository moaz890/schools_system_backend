...
export class GradeLevelsController {
    ...
    @ApiOperation({ summary: 'Create a grade level (school_admin only, auto-named for non-KG)' })
    ...
    @Get()
    @Roles(UserRole.SCHOOL_ADMIN)
    @ApiOperation({ summary: 'List grade levels, optionally filter by stage_id (school_admin only)' })
