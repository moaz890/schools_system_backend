...
exports.UuidPrimaryKeysSchoolEmailCascade1742600000000 = void 0;
class UuidPrimaryKeysSchoolEmailCascade1742600000000 {
    ...
    async up(queryRunner) {
        ...
        const idCol = await queryRunner.query(`SELECT data_type FROM information_schema.columns
        ...
        if (psExists) {
            ...
            await queryRunner.query(`ALTER TABLE "parent_student" RENAME COLUMN "id_new" TO "id"`);
            await queryRunner.query(`ALTER TABLE "parent_student" RENAME COLUMN "parent_id_new" TO "parent_id"`);
            await queryRunner.query(`ALTER TABLE "parent_student" RENAME COLUMN "student_id_new" TO "student_id"`);
        }
        ...
        await queryRunner.query(`ALTER TABLE "users" RENAME COLUMN "school_id_new" TO "school_id"`);
        ...
        if (psExists) {
            await queryRunner.query(`ALTER TABLE "parent_student" ALTER COLUMN "id" SET DEFAULT gen_random_uuid()`);
        }
        await queryRunner.query(`ALTER TABLE "schools" ADD CONSTRAINT "PK_schools" PRIMARY KEY ("id")`);
        await queryRunner.query(`ALTER TABLE "users" ADD CONSTRAINT "PK_users" PRIMARY KEY ("id")`);
        await queryRunner.query(`ALTER TABLE "sessions" ADD CONSTRAINT "PK_sessions" PRIMARY KEY ("id")`);
        if (psExists) {
            await queryRunner.query(`ALTER TABLE "parent_student" ADD CONSTRAINT "PK_parent_student" PRIMARY KEY ("id")`);
        }
        await queryRunner.query(`
            ALTER TABLE "users"
    ...
    async down(_queryRunner) {
        throw new Error('Down migration not supported: restores integer PKs and data mapping is lossy.');
    }
    async tableExists(queryRunner, name) {
        const r = await queryRunner.query(`SELECT to_regclass($1) AS reg`, [`public.${name}`]);
        return r[0]?.reg != null;
    }
    async dropForeignKeys(queryRunner, table) {
    ...
    async dropPrimaryKeys(queryRunner, table) {
        const exists = await this.tableExists(queryRunner, table);
        if (!exists)
            return;
        const rows = await queryRunner.query(`
    ...
    async dropSchoolEmailUniqueIfSingleColumn(queryRunner) {
        const rows = await queryRunner.query(`
            SELECT c.conname
            FROM pg_constraint c
            JOIN pg_class t ON c.conrelid = t.oid
        ...
        `);
        for (const { conname } of rows) {
            await queryRunner.query(`ALTER TABLE "schools" DROP CONSTRAINT "${conname}"`);
        }
    }
}
exports.UuidPrimaryKeysSchoolEmailCascade1742600000000 = UuidPrimaryKeysSchoolEmailCascade1742600000000;
//# sourceMappingURL=1742600000000-UuidPrimaryKeysSchoolEmailCascade.js.map