...
export class AcademicYearsController {
    ...
    create(@Body() dto: CreateAcademicYearDto, @CurrentUser() caller: AuthCaller) {
    ...
    get(@Param('id', ParseUUIDPipe) id: string, @CurrentUser() caller: AuthCaller) {
    ...
    @Roles(UserRole.SCHOOL_ADMIN)
    @ApiOperation({ summary: 'Mark academic year as current (and current semester by date)' })
    setCurrent(
        @Param('id', ParseUUIDPipe) id: string,
        @CurrentUser() caller: AuthCaller,
    ) {
