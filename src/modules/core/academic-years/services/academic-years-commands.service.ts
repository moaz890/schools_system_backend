...
export class AcademicYearsCommandsService {
  ...
  async updateYear(id: string, dto: UpdateAcademicYearDto, caller: AuthCaller) {
    ...
    return this.yearsRepo.manager.transaction(async (manager) => {
      ...
      await this.flags.repairCurrentByNow(manager, year.schoolId);
      return this.writerDal.findYearForRead(manager, id, scope.schoolId!);
    });
  }
  ...
  async setCurrentAcademicYear(id: string, caller: AuthCaller) {
    ...
    return this.yearsRepo.manager.transaction(async (manager) => {
      ...
      const now = new Date();
      const semester = await this.flags.findSemesterByDate(
        manager,
        year.id,
        now,
      );
  ...
  async createSemester(
    ...
    return this.yearsRepo.manager.transaction(async (manager) => {
      ...
      const semester = Array.isArray(saved) ? saved[0] : saved;
  ...
  async removeSemester(semesterId: string, caller: AuthCaller) {
    ...
    return this.yearsRepo.manager.transaction(async (manager) => {
      ...
      const yearSchoolId =
        semester.academicYear?.schoolId ?? semester.academicYearId;
      await manager.getRepository(Semester).softRemove(semester);
      await this.flags.repairCurrentByNow(manager, yearSchoolId);
    });
  }
  ...
  private scopeForCreate(
    explicitSchoolId: string | undefined,
    caller: AuthCaller,
  ): Scope {
    if (caller.role === UserRole.SUPER_ADMIN) {
      throw new ForbiddenException(
        'Super admin cannot create academic years/semesters',
      );
    }
    if (!caller.schoolId)
      throw new BadRequestException('You must be assigned to a school');
    if (explicitSchoolId !== caller.schoolId)
      throw new ForbiddenException(
        'You only can create academic years for your school',
      );
  ...
  private scopeForNonSuperAdmin(caller: AuthCaller): Scope {
    if (caller.role === UserRole.SUPER_ADMIN) {
      throw new ForbiddenException(
        'Super admin cannot modify academic years/semesters',
      );
    }
    if (!caller.schoolId) {
      throw new BadRequestException('You must be assigned to a school');
  ...
  private assertSemesterContainsNowForMarkCurrent(
    startDate: Date,
    endDate: Date,
  ) {
    const now = new Date();
  ...
  private async assertNewSemestersWithinYear(
    ...
    const year = await manager
      ...
      .findOne({ where: { id: yearId } as any });
    if (!year) throw new NotFoundException('Academic year not found');
    semesters.forEach((s) =>
      this.assertSemesterWithinYear(year, s.startDate, s.endDate),
    );
  }

  private assertNoOverlapInNewSemesters(
