...
export class AuthPasswordService {
  ...
  async changePassword(
    ...
    await this.authDal.updatePasswordAndCredentialVersion(
      userId,
      newHash,
      nextCv,
    );
  ...
  async forgotPassword(
    ...
    const user = await this.resolveUserForPasswordReset(
      dto.email,
      dto.schoolCode,
    );
    ...
    if (!user) {
      this.logger.log(
        'AuthPasswordService',
        'Forgot password: no matching user (generic response)',
        {
    ...
    if (user.status !== AccountStatus.ACTIVE) {
      this.logger.log(
        'AuthPasswordService',
        'Forgot password: user not active (generic response)',
        {
    ...
    if (user.schoolId && user.school && !user.school.isActive) {
      this.logger.log(
        'AuthPasswordService',
        'Forgot password: school inactive (generic response)',
        {
    ...
    try {
      ...
      this.logger.error(
        'AuthPasswordService',
        'Forgot password: transaction or email failed',
        {
    ...
    this.logger.log(
      'AuthPasswordService',
      'Forgot password: reset email flow completed',
      {
